...
/*   Updated: 2026/07/03 16:07:06 by alejandro        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::_handleClient(int fd)
{
	...
	if (bytes == 0)
	{
		if (_clients.find(fd) != _clients.end() && _clients[fd]->hasPendingBuffer())
			_clients[fd]->appendBuffer("\r\n", 2);

		std::string line;
		while (_clients.find(fd) != _clients.end() && _clients[fd]->getLine(line))
...
void Server::_cmdJoin(Client& client, const std::vector<std::string>& params)
{
	...
	// Si es el primer usuario, notificar al bot sobre el nuevo canal
	if (chan->memberCount() == 1)
	{
		chan->addOperator(&client);
		if (_bot)
			_bot->onChannelCreated(chan);
	}
						 ...
						 const std::string& trailing)
{
	...
	if (target[0] == '#') // mensaje a canal
	{
		...
		// Notificar al bot sobre el mensaje
		if (_bot)
			_bot->onMessageReceived(chan, client.getNick(), trailing);
